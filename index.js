...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Olá @${num.split('@')[0]}\nBem vindo ao grupo membro comum *${mdata.subject}*\n Num vai ser ghost hein fi da mãe kkkk`
				...
				teks = `Tchau @${num.split('@')[0]} ja foi tarde KKKK`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛Espere umpouco, a figurinha está sendo processada⌛',
				...
				only: {
					...
					Badmin: '❌ Este comando só pode ser usado quando o bot se torna um adm! ❌'
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["5591992142434@s.whatsapp.net"] // replace this with your number
			...
			switch(command) {
				...
				case 'dono':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* Asta\n*YOUTUBE:* https://youtube.com/channel/UCadqeKmUQTrggZt4bByzAJA\n*WPP:* wa.me/+5591992142434\n*INSTA:* @asta_ffyt\n\n\n*Digite *.bot* para ver comandos basicos para criar um bot'})
					break
				case 'belle2':
					memein = await kagApi.memeindo()
				...
				case 'belle1':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'to de zoio'})
					break
				case 'mia':
					memein = await kagApi.memeindo()
        ...
        case 'promote':
				...
				case 'kick':
					...
					if (mentioned.length > 1) {
						...
						mentions(`KKKKKKKKKK acabou de levar ban, bobão : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmins':
				...
				case 'simih':
					...
					if (Number(args[0]) === 1) {
						...
						reply('1 para ativar, 0 para desativar, lerdão você em ksksks🤦')
					}
					break
				case 'welcome':
					if (!isGroup) return reply(mess.only.group)
